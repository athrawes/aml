...
export function ast(tokens: Token[], level = 0): [Expression[], Expression[], number, Token[]] {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.kind === 'stringOpen') {
            let [exprs, unattactedSubExpressions, subLevel, remainingTokens] = ast(tokens.slice(i + 1), level);
            let expr = exprs.pop()
            ...
            unattachedExpressions.push(...unattactedSubExpressions);
        ...
        if (token.kind === 'stringClose') {
            root?.expression.push(token)
            return [attachedExpressions, unattachedExpressions, level, tokens.slice(i + 1)]
...
function expressionContainsBinding(expression: Expression): boolean {
    for (const e of expression.expression) {
        if ((!isExpression(e) && e.kind === 'binding')
